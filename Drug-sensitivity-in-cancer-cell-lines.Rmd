...
```
```{r variance filtering for clustering, echo=FALSE}
...
```
The following plot shows that there is no amount of clusters that is by far the best for clustering prism.achilles by cell lines but that two Clusters is probably our best option according to a slight kink at two clusters.
```{r, echo=FALSE}
