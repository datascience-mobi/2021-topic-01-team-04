...
Here we tried to display the data containing the different treatment effects for each cell lines via a heatmap. 
...
print(dr.to.effect(as.vector(ef.pancan.perdrug), doscor = 2)[1:3])
```
Here we can see that exatecan-mesylate is the drug that has the highest effect per dosage across all dosages. 
...
From this we can see that a clustering with two clusters is optimal.
