---
title: "DataCleanupTreatment"
author: "Autumn"
date: "02/06/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

globalobjectremover <- function(incfuncs = F) {
  if (incfuncs == F) {
    rm(list = setdiff(ls(pos = ".GlobalEnv"), lsf.str(pos = ".GlobalEnv")), pos = ".GlobalEnv")
  } else if (incfuncs == T) {
    rm(list = ls(pos = ".GlobalEnv"), pos = ".GlobalEnv")
  } else {
    print("Error>> incfuncs has wrong variable type. Use a boolean value.")
  }
}

globalobjectremover(T)
```


```{r}
load("../../InformaticsProjectData/prism_datasets.rda")
load("../../InformaticsProjectData/cellline_datasets.rda")

# equalising rownames
rownames(prism.cl) <- prism.cl[, "DepMap_ID"]
```


```{r}
# custom functions

# lateral is.na
lat.is.na <- function(x, TF = F) {
  r <- as.vector(sapply(x, is.na))
  if (TF) {
    return(r)
  } else if (!TF) {
    return(!r)
  } else {
    return("ERROR!")
  }
}

# frame is.na
frame.is.na <- function(X, TF = F) {
  R <- as.data.frame(matrix(nrow = nrow(X), ncol = ncol(X)))
  rownames(R) <- rownames(X); colnames(R) <- colnames(X)
  for (i in 1:nrow(X)) {
    R[i, ] <- lat.is.na(X[i, ], TF)
  }
  return(R)
}

# cleaner
row.col.cleaner <- function(X) {
  fin <- frame.is.na(X)
  fin1 <- as.vector(rev(which(apply(fin, 1, function(a) {sum(!a)}) == ncol(X))))
  fin2 <- as.vector(rev(which(apply(fin, 2, function(b) {sum(!b)}) == nrow(X))))
  if (length(fin1) > 0) {
    if (length(fin2) > 0) {
      return(list(X[-fin1, -fin2], fin[-fin1, -fin2]))
    } else if (length(fin2) == 0) {
      return(list(X[-fin1, ], fin[-fin1, ]))
    }
  } else if (length(fin1) == 0) {
    if (length(fin2) > 0) {
      return(list(X[, -fin2], fin[, -fin2]))
    } else if (length(fin2) == 0) {
      return(list(X, fin))
    }
  } else {
    return("ERROR! Please refer to the source code.")
  }
}

# extractor      NEED TO FIX, EXTRACT PANCAN FROM PRISM CORRECTLY WITH ASSOCIATED ROWNAMES, ROWNAMES PROBABLY TO BE TAKEN FROM PRISM, ISOLATION INSTRUCTION FROM PRISM.CL
prism.extractor <- function(X, phrase = "Pancreatic Cancer") {
  rn <- rownames(prism.cl)
  R <- X[rn, ]
  rownames(R) <- rn
  return(R)
}

#data frame list extractor
l.prism.extractor <- function(X, phrase = "Pancreatic Cancer") {
  R <- list()
  for (i in 1:length(X)) {
    R <- append(R, prism.extractor(X[i]))
  }
  return(R)
}


# make extractor also able to extracted from pre-cleaned prism data frames

```

```{r}
# deleting information-void rows and columns from prism

clean <- row.col.cleaner(prism)
prism.clean <- as.data.frame(clean[1])
prism.fin.clean <- as.data.frame(clean[2])
rm(clean)
```


```{r}
# extract pancreatic cancer cell line data

pancan.clean <- prism.extractor(prism.clean)
pancan.fin.clean <- prism.extractor(prism.fin.clean)
```


```{r}

```


```{r}
tata <- c()
for (i in 0:length(rownames(pancan.clean))) {
    tata <- append(tata, which(rownames(prism.cl) == rownames(pancan.clean)[i]))
}
tata
rownames(pancan.clean)
```