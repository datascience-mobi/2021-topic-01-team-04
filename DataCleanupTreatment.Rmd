...
```{r}
hist(apply(df.NA.to.val(pancan.clean, 2, "norm"), 2, mean), breaks = 20, main = "Drug Efficacy across Pancreatic Cancer Cell Lines", xlab = "Mean Logfold Drug Effect on Cell Growth", ylab = "Number of Drugs")
```
...
```{r}
hist(apply(df.NA.to.val(`Pancreatic Cancer Subtype: Ductal Adenocarcinoma, exocrine`, 2, "norm"), 2, mean), breaks = 20, main = "Drug Efficacy across Exocrine Ductal Adenocarcinoma Cell Lines", xlab = "Mean Logfold Drug Effect on Cell Growth", ylab = "Number of Drugs")
```
