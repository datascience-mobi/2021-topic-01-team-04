...
```{r}
unique(prism.treat[factor(names(which(apply(df.NA.to.val(prism.clean, 2, "norm"), 2, mean) < 0))), "name"])
```
...
```{r}
unique(prism.treat[factor(names(which(apply(df.NA.to.val(pancan.clean, 2, "norm"), 2, mean) < 0))), "name"])
