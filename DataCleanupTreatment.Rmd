...
cleaner <- function(X) {
  fin <- frame.is.na(X)
  fin1 <- which(apply(fin, 1, function(a) {sum(!a)}) == ncol(X))
  fin2 <- which(apply(fin, 2, function(b) {sum(!b)}) == nrow(X))
  fin1 <- rev(fin1)
  fin2 <- rev(fin2)
  if (length(fin1) > 0) {
    if (length(fin2) > 0) {
...
# extractor
extractor <- function(X, phrase = "Pancreatic Cancer") {
...
```{r}
frame.is.na(prism.clean)
frame.is.na(pancan.clean)
```
