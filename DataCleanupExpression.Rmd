...
for(g in 1:q)
  {
    ...
    boolean.p.vector = p.vector <= alpha
    boolean.p.vector[is.na(boolean.p.vector)] = FALSE
    zweiinteresting.genes = append(zweiinteresting.genes, colnames(prism.exp.pan[,boolean.p.vector]))
  }
```
```{r}
zwei.table.interesting.genes = table(zweiinteresting.genes)
zwei.df.interesting.genes = as.data.frame(zwei.table.interesting.genes)
```
